...
router.get('/expenses/:from/:to', function(req, res, next) {
    db.query(`SELECT id_ledger_group, name, sum(amount) from (
...
router.get('/payroll/:from/:to', function(req, res, next) {
    db.query(`SELECT SUM(amount) from ( 
...
router.get('/invoicePackingExp/:from/:to', function(req, res, next) {
    ...
    (select sum(amount) as amount, id_invoice from  payroll p, account_head a, invoice i where p.id_account_head=a.id_account_head and i.id_invoice=p.id_invoice and i.date between ${req.params.from} and ${req.params.to}
    union
    select sum(i.qty*p.selling_price) as amount, id_invoice from  invoice_packing_item i, product p, invoice i where p.id_product=i.id_product and i.id_invoice=p.id_invoice and i.date between ${req.params.from} and ${req.params.to} ) tbl`, function (err, rows, fields) {
      if (err) throw err
