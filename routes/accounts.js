...
router.get('/ledgerReport/:from_date/:to_date/:id_ledger/:activePage/:all?', function(req, res, next) {
 // console.log(req.params)
  ...
  db.query(`select id_ledger_group from account_head where id_account_head=${id_account_head}`, function (err, rows, fields) {
    ...
    var qryPurchase = (ledger_type == Constants.SUPPLIER) ? ` union select '3' as slno, '2' as vchr_type, id_purchase_voucher as id_voucher,DATE_FORMAT(date ,'%d/%m/%Y') as date ,date as _date,narration,'Purchase' as type,cast(gross as char) as receipt,cast(payable+discount as char) as payment from z_purchase_voucher  where date between ${from_date} and ${to_date} and id_account_head=${id_account_head}` : ``;
    ...
    var qryPayroll  = (ledger_type == Constants.STAFF) ? ` union select '3' as slno, '4' as vchr_type, id_payroll as id_voucher,DATE_FORMAT(date ,'%d/%m/%Y') as date ,date as _date,type as narration,'Payroll' as type,cast(amount as char) as receipt,'0' as payment from z_payroll  where date between ${from_date} and ${to_date}  and id_account_head=${id_account_head}` : ``;
        ...
        qryPayroll += (ledger_type == Constants.STAFF) ? ` union select '10' as slno, '4' as vchr_type, id_payroll as id_voucher,DATE_FORMAT(p.date ,'%d/%m/%Y') as date ,p.date as _date, p.type as narration,'Payroll' as type,cast(p.amount as char) as receipt,'0' as payment from payroll p where p.date between ${from_date} and ${to_date}  and p.id_account_head=${id_account_head} and p.id_invoice=0 ` : ``;
  ...
  console.log(req.params)
   ...
   // var to_date = req.params.to_date;
    ...
    db.query(`select id_ledger_group from account_head where id_account_head=${id_account_head}`, function (err, rows, fields) {
      ...
      var qryPurchase = (ledger_type == Constants.SUPPLIER) ? ` union select '3' as slno, '2' as vchr_type, id_purchase_voucher as id_voucher,DATE_FORMAT(date ,'%d/%m/%Y') as date ,date as _date,narration,'Purchase' as type,cast(gross as char) as receipt,cast(payable+discount as char) as payment from z_purchase_voucher  where date < ${from_date} and id_account_head=${id_account_head}` : ``;
      var qrySales = (ledger_type == Constants.SUPPLIER || ledger_type == Constants.CONSIGNER)
      ...
      var qryPayroll  = (ledger_type == Constants.STAFF) ? ` union select '3' as slno, '4' as vchr_type, id_payroll as id_voucher,DATE_FORMAT(date ,'%d/%m/%Y') as date ,date as _date,type as narration,'Payroll' as type,cast(amount as char) as receipt,'0' as payment from z_payroll  where date < ${from_date} and id_account_head=${id_account_head}` : ``;
          ...
          qryPayroll += (ledger_type == Constants.STAFF) ? ` union select '10' as slno, '4' as vchr_type, id_payroll as id_voucher,DATE_FORMAT(p.date ,'%d/%m/%Y') as date ,p.date as _date, p.type as narration,'Payroll' as type,cast(p.amount as char) as receipt,'0' as payment from payroll p where p.date < ${from_date} and p.id_account_head=${id_account_head} and p.id_invoice=0 ` : ``;
